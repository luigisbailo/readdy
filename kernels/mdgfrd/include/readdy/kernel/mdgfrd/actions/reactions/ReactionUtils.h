...
using mdgfrd_kernel = readdy::kernel::mdgfrd::MDGFRDKernel;
using reaction_type = readdy::model::reactions::ReactionType;
using ctx_t = std::remove_const<decltype(std::declval<mdgfrd_kernel>().context())>::type;
using event_t = Event;
using record_t = readdy::model::reactions::ReactionRecord;
using reaction_counts_map = MDGFRDStateModel::reaction_counts_map;
...
data_t::DataUpdate handleEventsGillespie(
        MDGFRDKernel* kernel, readdy::scalar timeStep,
...
void gatherEvents(MDGFRDKernel *const kernel, const ParticleIndexCollection &particles, const neighbor_list* nl,
