...
//    void assignMaxDomainSize (scalar domainSize, size_type index){
//        this->entry_at(index).maxDomainSize = domainSize;
//    }
//
//    void assignMinDomainSize (scalar domainSize, size_type index){
//        this->entry_at(index).minDomainSize = domainSize;
//    }
...
struct Entry {
    ...
    explicit Entry(const Particle &particle)
            : pos(particle.getPos()), force(), type(particle.getType()), id(particle.getId()),
    simulationTime(0), constructionTime(), exitTime(),
              deactivated(false), GFpropagation(false), GFend(false){};
