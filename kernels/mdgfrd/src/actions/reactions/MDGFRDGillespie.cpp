...
MDGFRDGillespie::MDGFRDGillespie(MDGFRDKernel *const kernel, scalar timeStep) : super(timeStep), kernel(kernel) {}

void MDGFRDGillespie::perform(const util::PerformanceNode &node) {
    auto t = node.timeit();
    const auto &ctx = kernel->context();
    if(ctx.reactions().nOrder1() == 0 && ctx.reactions().nOrder2() == 0) {
        return;
    }
    auto &stateModel = kernel->MDGFRDKernelStateModel();
