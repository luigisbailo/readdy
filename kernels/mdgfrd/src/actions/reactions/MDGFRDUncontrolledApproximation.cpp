...
#include <readdy/kernel/mdgfrd/actions/reactions/Event.h>
#include <readdy/kernel/mdgfrd/actions/reactions/ReactionUtils.h>
#include <readdy/kernel/mdgfrd/data/DataContainer.h>
...
using neighbor_list = MDGFRDStateModel::neighbor_list;
...
MDGFRDUncontrolledApproximation::MDGFRDUncontrolledApproximation(MDGFRDKernel *const kernel, scalar timeStep)
        : super(timeStep), kernel(kernel) {

}

void findEvents(std::size_t /*tid*/, data_iter_t begin, data_iter_t end, nl_bounds nlBounds,
                const MDGFRDKernel *const kernel, scalar dt, bool approximateRate, const neighbor_list &nl,
...
void MDGFRDUncontrolledApproximation::perform(const util::PerformanceNode &node) {
    auto t = node.timeit();
    ...
    if (ctx.recordReactionsWithPositions()) {
        kernel->getMDGFRDKernelStateModel().reactionRecords().clear();
    }
            ...
            if (event.cumulativeRate == 0) {
                ...
                if (event.nEducts == 1) {
                    ...
                    if (ctx.recordReactionsWithPositions()) {
                        ...
                        fixPos(record.where);
                        kernel->getMDGFRDKernelStateModel().reactionRecords().push_back(record);
                    } else {
